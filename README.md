# Hi, I'm Anirudh

Senior backend/platform engineer focused on OCI, database reliability, backup/recovery, and distributed systems.

Most of my production work has been in proprietary Oracle cloud infrastructure, so that code is not public here. This profile is where I share the kinds of engineering problems I enjoy, the public projects I build, and the ideas I'm exploring.

## What I work on
- distributed systems and platform engineering
- database reliability, backup, recovery, and high availability
- workflow orchestration and long-running jobs
- developer tooling, debugging, and operational automation
- AI-assisted workflows for engineering productivity

## Oracle Cloud areas I know well
- [Oracle Base Database Service](https://docs.oracle.com/en/cloud/paas/base-database/about/index.html)
- [Back Up and Recovery in Base Database Service](https://docs.oracle.com/en/cloud/paas/base-database/backup-recover/index.html)
- [Oracle Data Guard on Base Database Service](https://docs.oracle.com/en/cloud/paas/base-database/data-guard/index.html)
- [Database Autonomous Recovery Service](https://docs.oracle.com/en-us/iaas/recovery-service/index.html)
- [OCI Vault](https://docs.oracle.com/en-us/iaas/Content/KeyManagement/home.htm)

## What you'll find here
- public projects inspired by real backend and platform problems
- polished repos with clear documentation and practical scope
- utilities and experiments in engineering productivity and automation
- technical notes and write-ups on reliability-oriented engineering

## Current focus
I'm building more public-facing projects and writing that reflect the systems work I do professionally, without exposing proprietary code.

Right now I'm especially interested in:
- reliability engineering for database and platform systems
- operational tooling and debugging workflows
- AI-assisted developer tools and agentic engineering

## Stats
![Anirudh's GitHub stats](https://github-readme-stats.vercel.app/api?username=anirudhmehra&show_icons=true&hide_border=true&rank_icon=github)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=anirudhmehra&layout=compact&hide_border=true)
